  ...
  return <div className="flo-app-shell">
    <aside className={`flo-sidebar ${navCollapsed ? 'is-collapsed' : ''}`}>
      <div className="flo-brand-row">
        <button onClick={() => navigate('/dashboard')} className="flo-brand"><span className="flo-brand-mark">F</span>{!navCollapsed && <span className="flo-brand-copy"><strong className="flo-brand-name">flo<em>studio</em></strong><small className="flo-brand-caption">CREATIVE OS</small></span>}</button>
      ...
      <button onClick={() => setCopilotOpen(true)} className="flo-ask" style={{ justifyContent:navCollapsed ? 'center':'flex-start', padding:navCollapsed ? '12px':'12px 14px' }}><span className="flo-ask-mark">F</span>{!navCollapsed && <span className="flo-ask-copy">Ask Flo anything</span>}</button>
      ...
      <div className="flo-sidebar-footer">{!navCollapsed && <div className="flo-fuel"><div className="flo-fuel-label">CREATIVE FUEL</div><div className="flo-fuel-amount"><b>{tokens}</b><span>tokens</span></div></div>}<div className="flo-profile" style={{ justifyContent:navCollapsed ? 'center':'flex-start' }}><div className="flo-avatar">{initial}</div>{!navCollapsed && <div className="flo-profile-copy" style={{ minWidth:0, flex:1 }}><div className="flo-profile-email">{user?.email || 'FloStudio user'}</div><div className="flo-profile-tier">{tier} workspace</div></div>}{!navCollapsed && <button onClick={logout} className="flo-signout" title="Sign out">→</button>}</div></div>
    </aside>
    <main className={`flo-main ${navCollapsed ? 'is-collapsed' : ''}`}>
      <header className="flo-topbar"><div><div className="flo-topbar-crumb">FLOSTUDIO / {meta[0]}</div><div className="flo-topbar-meta">{meta[1]}</div></div><div className="flo-topbar-actions"><button onClick={() => setCopilotOpen(true)} className="flo-topbar-copilot">Flo Copilot</button><button onClick={() => navigate('/pricing')} className="flo-topbar-tokens">{tokens} tokens</button></div></header>
