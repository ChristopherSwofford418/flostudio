...
export default function AppAwarePostStudio({ apps = [], workspaceId }) {
  ...
  return <section className="studio-panel" style={{ marginTop:22, padding:24, background:'linear-gradient(145deg,rgba(21,24,45,.98),rgba(37,40,83,.96))', borderColor:'rgba(167,161,255,.38)' }}>
    ...
    {drafts.length > 0 && <div style={{ marginTop:22, paddingTop:18, borderTop:'1px solid rgba(255,255,255,.13)' }}><div className="studio-kicker" style={{ color:'rgba(204,200,255,.88)' }}>REVIEW-READY DRAFTS</div><div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(290px,1fr))', gap:12, marginTop:12 }}>{drafts.map(draft => <article key={draft.id} style={{ padding:15, borderRadius:14, background:'#fff', color:'#20243d' }}><div style={{ display:'flex', justifyContent:'space-between', gap:8, alignItems:'center' }}><b style={{ fontSize:12, textTransform:'capitalize' }}>{draft.platform}</b><span className="studio-chip" style={{ color:'#4c46cd', background:'rgba(95,89,232,.08)', borderColor:'rgba(95,89,232,.2)' }}>{draft.status?.replaceAll('_',' ')}</span></div>{draft.hook && <p style={{ color:'#525973', fontWeight:800, fontSize:12, lineHeight:1.45, marginTop:10 }}>{draft.hook}</p>}<p style={{ color:'#30364f', fontSize:12, lineHeight:1.65, whiteSpace:'pre-wrap', marginTop:9 }}>{draft.caption}</p><p style={{ color:'#5f59e8', fontSize:10.5, lineHeight:1.6, marginTop:10 }}>{displayHashtags(draft.hashtags)}</p>{draft.call_to_action && <div style={{ marginTop:10, paddingTop:9, borderTop:'1px solid #e7e8ef', color:'#6b7288', fontSize:10.5 }}>CTA · {draft.call_to_action}</div>}</article>)}</div></div>}
  </section>
}
