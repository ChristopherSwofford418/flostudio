...
export default function AppAwarePostStudio({ apps = [], workspaceId, activeAppId: controlledAppId = '', onAppChange }) {
  ...
  return <section className="studio-panel" style={{ marginTop:22, padding:24, background:'linear-gradient(145deg,rgba(21,24,45,.98),rgba(37,40,83,.96))', borderColor:'rgba(167,161,255,.38)' }}>
    <style>{`
      ...
      .app-aware-post-controls { display:grid; grid-template-columns:minmax(0,1fr) 150px 150px; gap:10px; }
      ...
      @media (max-width:700px) { .app-aware-post-controls { grid-template-columns:1fr; } }
    `}</style>
    ...
    <div className="app-aware-post-layout">
      <div className="app-aware-post-main">
        <div className="app-aware-post-controls"><label style={{ display:'grid', gap:5, color:'rgba(228,229,255,.8)', fontSize:9.5, fontWeight:850, letterSpacing:'.09em', textTransform:'uppercase' }}>Portfolio app<select value={app?.id || ''} onChange={event => changeApp(event.target.value)} style={{ padding:'9px 11px', borderRadius:10, color:'#222842', background:'#fff' }}>{apps.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label style={{ display:'grid', gap:5, color:'rgba(228,229,255,.8)', fontSize:9.5, fontWeight:850, letterSpacing:'.09em', textTransform:'uppercase' }}>Channel<select value={platform} onChange={event => setPlatform(event.target.value)} style={{ padding:'9px 11px', borderRadius:10, color:'#222842', background:'#fff' }}>{PLATFORM_OPTIONS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label style={{ display:'grid', gap:5, color:'rgba(228,229,255,.8)', fontSize:9.5, fontWeight:850, letterSpacing:'.09em', textTransform:'uppercase' }}>Asset type<select value={mediaKind} onChange={event => setMediaKind(event.target.value)} style={{ padding:'9px 11px', borderRadius:10, color:'#222842', background:'#fff' }}><option value="image">Image post</option><option value="video">Video post</option><option value="text">Text-led post</option></select></label></div>
