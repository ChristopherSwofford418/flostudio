...
import { setRunbookProgressVisibility, setRunbookStatus } from '../lib/campaignRunbook'
...
export default function CampaignMomentumMap({ runbook, checkpoints = [], nextAction, onNavigate, onChanged }) {
  ...
  if (!runbook) return <section className="studio-panel" style={{ padding:18, borderColor:'rgba(184,175,255,.28)', background:'linear-gradient(135deg,rgba(83,67,153,.22),rgba(20,21,42,.9))' }}><div className="studio-kicker" style={{ color:'#c9c2ff' }}>CAMPAIGN RUNBOOK</div><h3 style={{ color:'#fff', fontSize:18, marginTop:6 }}>The runbook begins when a campaign is saved.</h3><p style={{ color:'rgba(242,243,255,.7)', fontSize:11.5, lineHeight:1.6, marginTop:6 }}>Flo will map only product facts, selected theses, completed assets, human reviews, controlled tests, and sourced learning. No clicks, token activity, or generation requests advance it.</p></section>
