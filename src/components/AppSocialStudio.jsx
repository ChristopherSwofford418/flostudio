...
export default function AppSocialStudio({ apps = [], workspaceId }) {
  ...
  useEffect(() => {
    if (apps.length && !activeAppId) setActiveAppId(apps[0].id)
  }, [apps, activeAppId])
  useEffect(() => {
  ...
  return <section className="studio-panel" style={{ marginTop:22, padding:24, border:'1px solid rgba(99,91,255,.26)', background:'linear-gradient(145deg,rgba(255,255,255,.98),rgba(244,246,255,.94))' }}>
    <div style={{ display:'flex', justifyContent:'space-between', gap:16, alignItems:'flex-start', flexWrap:'wrap' }}>
      ...
      <div className="studio-chip" style={{ borderColor:status?.configured ? 'rgba(35,176,143,.45)' : 'rgba(216,151,53,.48)', color:status?.configured ? '#13856d' : '#a86211', background:status?.configured ? 'rgba(35,176,143,.1)' : 'rgba(216,151,53,.11)' }}>{status?.configured ? 'UNIFIED SOCIAL READY' : 'PROVIDER SETUP REQUIRED'}</div>
    </div>
