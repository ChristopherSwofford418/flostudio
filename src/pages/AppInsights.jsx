...
import { appStoreConnectPath } from '../lib/appStoreConnectRouting'
...
export default function AppInsights() {
  ...
  return <Layout title="App Insights"><div className="page-shell" style={{ padding:'28px 30px 52px' }}>
    ...
    {state.loading ? <section className="studio-panel" style={{ marginTop:16, padding:28, color:'rgba(243,240,231,.65)' }}>Loading this app’s secured reporting state…</section> : !connection?.status || connection.status !== 'connected' ? <section className="studio-panel" style={{ marginTop:16, padding:26, borderColor:'rgba(255,193,59,.3)', background:'rgba(112,35,9,.3)' }}><div className="studio-kicker" style={{ color:'var(--signal)' }}>APP STORE CONNECT NOT YET CONNECTED</div><h2 style={{ color:'#fff', fontSize:23, marginTop:7 }}>{selectedApp?.name} has no performance feed yet.</h2><p style={{ color:'rgba(243,240,231,.66)', fontSize:12, lineHeight:1.6, marginTop:8, maxWidth:700 }}>Open the portfolio connection panel, enter this app’s App Store Connect App ID and API Key ID, then drop the matching `.p8` file. FloStudio will validate the key, encrypt it server-side, and begin showing only authorized results here.</p><button onClick={() => navigate(appStoreConnectPath(selectedApp?.id))} className="studio-button" style={{ marginTop:15 }}>Connect {selectedApp?.name || 'this app'} →</button></section> : <>
    ...
    {state.notice && <div style={{ marginTop:14, padding:11, border:'1px solid rgba(255,193,59,.35)', background:'rgba(255,193,59,.08)', color:'#fff0c7', fontSize:12 }}>{state.notice}</div>}
  </div></Layout>
}
