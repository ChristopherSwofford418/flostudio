    ...
    <Layout title="Content Pipeline">
      ...
      <div className="flo-page" style={{ maxWidth: 1100, margin: '0 auto', animation: 'fadeIn 0.25s ease' }}>
        ...
        <div className="abundance-glass" style={{ display: 'flex', gap: 6, marginBottom: 20, borderRadius: 12, padding: 6, width: 'fit-content' }}>
          {STATUS_TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: '8px 18px', borderRadius: 8, background: activeTab === tab ? 'linear-gradient(135deg,#7b61ff,#ef4f9a)' : 'transparent', border: 'none', color: activeTab === tab ? '#fff' : 'rgba(234,229,255,.63)', fontSize: 13, fontWeight: activeTab === tab ? 700 : 500, cursor: 'pointer', fontFamily: 'inherit', textTransform: 'capitalize', transition: 'all 0.15s' }}>{tab}</button>
          ))}
        </div>
