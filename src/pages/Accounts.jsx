...
const PLATFORMS = [
  { id:'facebook', label:'Facebook Pages', icon:'F', color:'#5b8dff', summary:'Authorize a real Meta identity, then choose the Page that FloStudio may publish approved posts to.', requirement:'Requires Facebook Login for Business, Page access, approved Page permissions, and a secure server-side callback.' },
  { id:'instagram', label:'Instagram Professional', icon:'I', color:'#ff7ab6', summary:'Authorize a real Instagram Professional account connected to an eligible Page.', requirement:'Requires Meta app setup, an eligible Professional account, content publishing access, and an approved callback.' },
  { id:'linkedin', label:'LinkedIn', icon:'in', color:'#83d9ff', summary:'Authorize a real LinkedIn member first. Organization publishing remains unavailable until its separate approval is granted.', requirement:'Requires LinkedIn OAuth and the Share on LinkedIn product with w_member_social.' },
  { id:'tiktok', label:'TikTok', icon:'T', color:'#bff5e8', summary:'Authorize a real TikTok creator. Direct Post remains media-specific and subject to TikTok’s Content Posting audit.', requirement:'Requires Login Kit, Content Posting API approval, video.publish, and a secure callback.' },
  { id:'twitter', label:'X', icon:'X', color:'#d9ff75', summary:'Authorize a real X account using OAuth 2.0 with PKCE. Approved posts receive a real remote Post ID only after X accepts them.', requirement:'Requires an approved X developer app, exact callback URI, tweet.write, users.read, and server-side token exchange.' },
]
...
function toneFor(status) {
  if (status?.live && status?.status === 'connected') return '#d9ff75'
  if (status?.status === 'needs_reauthorization') return '#ffaccb'
  if (status?.configured) return '#83d9ff'
  return '#ffd480'
}
  ...
  return <Layout title="Channels">
    <div className="flo-page" style={{ maxWidth:1080, margin:'0 auto', padding:'28px 30px 56px' }}>
      <section className="studio-dark abundance-hero" style={{ minHeight:230, padding:'28px 30px', display:'grid', gridTemplateColumns:'minmax(0,1fr) 270px', gap:24, alignItems:'center', position:'relative', overflow:'hidden' }}>
        <div style={{ position:'relative', zIndex:1 }}><div className="studio-kicker" style={{ color:'#d9ff75' }}>DISTRIBUTION DESK / CONNECTION HEALTH</div><h1 className="studio-display" style={{ color:'#fff', fontSize:'clamp(34px,4vw,54px)', marginTop:10 }}>Publish only when the <span className="studio-serif" style={{ color:'#ffaccb' }}>connection is real.</span></h1><p style={{ maxWidth:620, marginTop:13, color:'rgba(255,250,244,.72)', fontSize:13, lineHeight:1.7 }}>FloStudio routes every connection through the provider’s real consent screen. It does not accept pasted tokens, fake account handles, or browser-side secret exchange—and it only marks a post published after a provider returns a real remote post ID.</p></div>
        <div className="studio-panel" style={{ position:'relative', zIndex:1, padding:18, background:'rgba(10,7,28,.48)', borderColor:'rgba(215,242,103,.22)' }}><div className="studio-kicker" style={{ color:'#d9ff75' }}>PORTFOLIO SCOPE</div><div style={{ color:'#fff', fontSize:26, fontWeight:850, marginTop:8 }}>{apps.length}</div><div style={{ color:'rgba(244,240,255,.6)', fontSize:11, lineHeight:1.5, marginTop:5 }}>apps ready for their own verified social destinations</div></div>
      ...
      <section style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginTop:16 }}>
        <div className="studio-panel" style={{ padding:17 }}><div className="studio-kicker" style={{ color:'#83d9ff' }}>01 / AUTHORIZE</div><p style={{ color:'rgba(244,240,255,.68)', fontSize:11.5, lineHeight:1.55, marginTop:8 }}>FloStudio opens the provider’s real consent screen, then completes the code exchange only on the server.</p></div>
        <div className="studio-panel" style={{ padding:17 }}><div className="studio-kicker" style={{ color:'#ffaccb' }}>02 / MAP</div><p style={{ color:'rgba(244,240,255,.68)', fontSize:11.5, lineHeight:1.55, marginTop:8 }}>When a provider returns several destinations, choose the specific Page or Professional account FloStudio may use.</p></div>
        <div className="studio-panel" style={{ padding:17 }}><div className="studio-kicker" style={{ color:'#d9ff75' }}>03 / PUBLISH</div><p style={{ color:'rgba(244,240,255,.68)', fontSize:11.5, lineHeight:1.55, marginTop:8 }}>Only an approved Review Queue post can start a real publish attempt. Provider success—not UI state—creates published status.</p></div>
      </section>

      {notice && <div style={{ marginTop:16, padding:13, borderRadius:11, background:'rgba(255,180,206,.1)', border:'1px solid rgba(255,180,206,.25)', color:'#ffd0df', fontSize:11.5, lineHeight:1.6 }}>{notice}</div>}
      <section style={{ marginTop:26 }}><div className="studio-kicker" style={{ color:'#ffaccb' }}>CHANNEL CONTROL</div><h2 style={{ color:'#fff', fontSize:25, letterSpacing:'-.05em', marginTop:5 }}>Authorize a channel. Then map the <span className="studio-serif" style={{ color:'#ffaccb' }}>real destination.</span></h2>
      ...
      {destinationSelection && <div style={{ position:'fixed', inset:0, zIndex:100, background:'rgba(3,18,14,.76)', backdropFilter:'blur(9px)', display:'grid', placeItems:'center', padding:20 }}><section className="studio-panel" style={{ width:'min(620px,100%)', padding:25, background:'#10372b', borderColor:'rgba(255,255,255,.18)' }}><div className="studio-kicker" style={{ color:'#d9ff75' }}>META DESTINATION SELECTION</div><h2 style={{ color:'#fff', fontSize:28, letterSpacing:'-.05em', marginTop:8 }}>Choose where FloStudio may <span className="studio-serif" style={{ color:'#ffaccb' }}>publish.</span></h2><p style={{ color:'rgba(244,240,255,.66)', marginTop:9, fontSize:12, lineHeight:1.65 }}>Meta returned these eligible destinations. FloStudio will save the one you select and keep its provider credential in the server-side credential vault.</p><div style={{ display:'grid', gap:9, marginTop:18 }}>{destinationSelection.destinations.map(destination => <button key={destination.id} onClick={() => selectDestination(destination)} disabled={checking === destinationSelection.platform} style={{ textAlign:'left', padding:'14px 16px', borderRadius:10, border:'1px solid rgba(255,255,255,.17)', background:'rgba(255,255,255,.06)', color:'#fff', cursor:'pointer' }}><div style={{ fontSize:14, fontWeight:850 }}>{destination.accountName}</div><div style={{ color:'rgba(244,240,255,.58)', fontSize:11, marginTop:4 }}>{destination.accountType}{destination.accountHandle ? ` · ${destination.accountHandle}` : ''}</div></button>)}</div><button onClick={() => setDestinationSelection(null)} className="studio-button studio-button--soft" style={{ marginTop:16, padding:'9px 12px', fontSize:11 }}>Cancel selection</button></section></div>}
    </div>
  </Layout>
}
