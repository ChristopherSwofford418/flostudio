...
export default function Accounts() {
  ...
  return <Layout title="Channels">
    <div className="flo-page channels-page" style={{ maxWidth:1080, margin:'0 auto', padding:'28px 30px 56px' }}>
      <section className="studio-dark abundance-hero" style={{ minHeight:230, padding:'28px 30px', display:'grid', gridTemplateColumns:'minmax(0,1fr) 270px', gap:24, alignItems:'center', position:'relative', overflow:'hidden' }}>
        ...
        <div className="studio-panel flo-dark-surface channels-scope-card" style={{ position:'relative', zIndex:1, padding:18, background:'rgba(9,9,9,.48)', borderColor:'rgba(226,226,226,.22)' }}><div className="studio-kicker" style={{ color:'#ededed' }}>PORTFOLIO SCOPE</div><div style={{ color:'#ffffff', fontSize:26, fontWeight:850, marginTop:8 }}>{apps.length}</div><div style={{ color:'rgba(242,242,242,.6)', fontSize:11, lineHeight:1.5, marginTop:5 }}>apps ready for their own verified social destinations</div></div>
      ...
      <details className="channels-advanced-direct" style={{ marginTop:16 }}><summary style={{ cursor:'pointer', color:'#4d5270', fontWeight:800, fontSize:12 }}>Advanced direct connections (not used for app-specific publishing)</summary><p style={{ color:'#69708b', fontSize:11, lineHeight:1.55, margin:'8px 0 12px' }}>Use the app-specific setup above for Facebook, Instagram, and X. These legacy direct connectors remain available for future developer-managed integrations but do not create an isolated profile for every portfolio app.</p>
      <section className="channels-legacy-explainer" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginTop:16 }}>
        <div className="studio-panel channels-step-card" style={{ padding:17 }}><div className="studio-kicker" style={{ color:'#c9c9c9' }}>01 / AUTHORIZE</div><p style={{ color:'rgba(242,242,242,.68)', fontSize:11.5, lineHeight:1.55, marginTop:8 }}>FloStudio opens the provider’s real consent screen, then completes the code exchange only on the server.</p></div>
        <div className="studio-panel channels-step-card" style={{ padding:17 }}><div className="studio-kicker" style={{ color:'#c0c0c0' }}>02 / MAP</div><p style={{ color:'rgba(242,242,242,.68)', fontSize:11.5, lineHeight:1.55, marginTop:8 }}>When a provider returns several destinations, choose the specific Page or Professional account FloStudio may use.</p></div>
        <div className="studio-panel channels-step-card" style={{ padding:17 }}><div className="studio-kicker" style={{ color:'#ededed' }}>03 / PUBLISH</div><p style={{ color:'rgba(242,242,242,.68)', fontSize:11.5, lineHeight:1.55, marginTop:8 }}>Only an approved Review Queue post can start a real publish attempt. Provider success—not UI state—creates published status.</p></div>
      </section>

      {notice && <div className="channels-notice" style={{ marginTop:16, padding:13, borderRadius:11, background:'rgba(198,198,198,.1)', border:'1px solid rgba(198,198,198,.25)', color:'#dbdbdb', fontSize:11.5, lineHeight:1.6 }}>{notice}</div>}
      <section className="channels-control" style={{ marginTop:26 }}><div className="studio-kicker" style={{ color:'#c0c0c0' }}>ADVANCED DIRECT CONNECTIONS</div><h2 style={{ color:'#ffffff', fontSize:25, letterSpacing:'-.05em', marginTop:5 }}>Developer-managed OAuth connections.</h2>
