...
const PLATFORMS = [
  { id: 'facebook', label: 'Facebook Page', description: 'Connect your Facebook Business or Fan Page for automated publishing', color: '#1877f2', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg> },
...
export default function Accounts() {
  ...
  const handleConnectSubmit = async (e) => {
    e.preventDefault()
    if (!modalPlatform || !accountNameInput.trim()) return
    ...
    try {
      // Upsert or insert connected account
      const { error } = await supabase.from('connected_accounts').insert({
        user_id: user.id,
        platform: modalPlatform.id,
        account_name: accountNameInput.trim(),
        account_handle: accountHandleInput.trim() || `@${accountNameInput.trim().toLowerCase().replace(/\s+/g, '')}`,
        access_token: 'mock_oauth_token_' + Math.random().toString(36).substring(7),
        status: 'connected',
      })

      if (error) throw error

      alert(`Successfully connected ${modalPlatform.label} (${accountNameInput.trim()})!`)
  ...
  return (
    <Layout title="Connected Accounts">
      <div style={{ maxWidth: 840, animation: 'fadeIn 0.3s ease' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, rgba(236,72,153,0.1), rgba(139,92,246,0.08))', border: '1px solid rgba(236,72,153,0.25)', borderRadius: 18, padding: '20px 24px', marginBottom: 32, display: 'flex', gap: 16, alignItems: 'center' }}>
          ...
          <div>
            ...
            <p style={{ color: '#94a3b8', fontSize: 13.5, lineHeight: 1.5 }}>
              Connect your professional social channels to allow FloStudio and the AI Agent to automatically publish and schedule posts across all platforms.
            </p>
          </div>
        </div>
        ...
        {/* Modal for OAuth Connection Simulation / Details */}
        {modalPlatform && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(7,11,25,0.85)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 20 }}>
            <div style={{ background: '#101932', border: '1px solid rgba(236,72,153,0.3)', borderRadius: 24, padding: 36, width: '100%', maxWidth: 460, boxShadow: '0 25px 60px rgba(0,0,0,0.8)', animation: 'fadeIn 0.2s ease' }}>
              ...
              <p style={{ color: '#94a3b8', fontSize: 13.5, marginBottom: 24, lineHeight: 1.5 }}>
                Authorize FloStudio to manage posts and analytics on your {modalPlatform.label} account securely via OAuth 2.0.
              </p>
              <form onSubmit={handleConnectSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#cbd5e1', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Account / Page Name</label>
                  <input type="text" value={accountNameInput} onChange={e=>setAccountNameInput(e.target.value)} required style={{ width: '100%', background: '#070b19', border: '1px solid rgba(236,72,153,0.3)', borderRadius: 10, padding: '12px 16px', color: '#f8fafc', fontSize: 14, fontFamily: 'inherit' }} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#cbd5e1', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Handle / Username</label>
                  <input type="text" value={accountHandleInput} onChange={e=>setAccountHandleInput(e.target.value)} required style={{ width: '100%', background: '#070b19', border: '1px solid rgba(236,72,153,0.3)', borderRadius: 10, padding: '12px 16px', color: '#f8fafc', fontSize: 14, fontFamily: 'inherit' }} />
                </div>
