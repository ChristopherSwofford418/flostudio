...
export default function ImageBank() {
  ...
  const startVideo = async () => {
    ...
    const runbookPrompt = `${selectedRunbook.label} format. ${videoPrompt || selectedRunbook.video} Hook: ${hook || 'derive a clear, credible hook.'} Proof: ${proof || 'show only supportable product truth.'}${productContext}`
    const request = { prompt:runbookPrompt, size:videoFormat, seconds:videoSeconds, quality:videoQuality === 'production' ? 'production' : 'draft', referenceImage }
    ...
    try {
      ...
      const normalized = { ...job, mediaAssetId:persisted.id, prompt:runbookPrompt, size:videoFormat, seconds:videoSeconds, status:job.status || 'queued', createdAt:Date.now() }
