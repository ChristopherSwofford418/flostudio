...
export default function ImageBank() {
  ...
  return <Layout title="Creative Lab">
    ...
    `}</style>
    <style>{`
      .flo-app-shell:has(.arcads-studio){--signal:#f6f6f2;--vermilion:#b8b8b4;--ink-deep:#090909;--mineral:#f4f4f0;--moss:#a8a8a1;background:#080808!important;color:#f4f4f0}
      ...
      .flo-app-shell:has(.arcads-studio) .flo-brand-mark{background:#f4f4f0!important;color:#080808!important;border-color:#f4f4f0!important}
      .flo-app-shell:has(.arcads-studio) .flo-nav-item.active{background:#f4f4f0!important;color:#080808!important}
      ...
      .flo-app-shell:has(.arcads-studio) .flo-fuel,.flo-app-shell:has(.arcads-studio) .flo-profile{background:#111!important;border-color:#2a2a2a!important}
      .arcads-studio{padding:0 22px 34px;color:#f4f4f0}
      ...
      .arcads-studio .product-truth{border-color:#2b2b2b!important}.arcads-studio .product-truth option{background:#111!important;color:#f4f4f0!important}
      .arcads-studio .media-rail-item,.arcads-studio .asset-card{background:#111!important;border-color:#2f2f2f!important}.arcads-studio .media-rail-item::after{background:#050505!important;color:#dcdcd8!important}.arcads-studio .asset-card .asset-footer{background:linear-gradient(transparent,rgba(4,4,4,.97) 48%)!important}
      .arcads-studio .abundance-card,.arcads-studio .abundance-glass{background:#101010!important;border-color:#2d2d2d!important;box-shadow:none!important}.arcads-studio .abundance-pill{background:#151515!important;border-color:#343434!important;color:#e6e6e2!important}
      .arcads-studio .format-card{background:#121212!important;border-color:#303030!important;color:#d8d8d4!important}.arcads-studio .format-card.active{background:#f4f4f0!important;border-color:#f4f4f0!important;color:#0b0b0b!important}.arcads-studio .format-card.active small{color:#464646!important}.arcads-studio .format-card small{color:#949490!important}
      .arcads-studio .runbook-card{background:#111!important;border-color:#303030!important;color:#e9e9e5!important}.arcads-studio .runbook-card.active{background:#f4f4f0!important;border-color:#f4f4f0!important;color:#0b0b0b!important;box-shadow:4px 4px 0 #4a4a4a!important}.arcads-studio .runbook-card__type{color:#a8a8a1!important}.arcads-studio .runbook-card.active .runbook-card__type,.arcads-studio .runbook-card.active small{color:#4c4c49!important}
      .arcads-studio .ad-blueprint{border-color:#363636!important;background:#121212!important}.arcads-studio .ad-blueprint input,.arcads-studio .studio-input{background:#080808!important;border-color:#343434!important;color:#f4f4f0!important}.arcads-studio .ad-blueprint input::placeholder,.arcads-studio .studio-input::placeholder{color:#7c7c78!important}
      .arcads-studio button.studio-button{background:#f4f4f0!important;color:#080808!important;border-color:#f4f4f0!important}.arcads-studio button.studio-chip{background:#151515!important;color:#e8e8e4!important;border-color:#404040!important}
    ...
    <div className="flo-page arcads-studio" style={{ maxWidth:1280, margin:'0 auto', animation:'fadeIn .3s ease-out' }}>
      ...
      {activeTab === 'library' && <section className="abundance-card" style={{ padding:23 }}><div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', gap:16, marginBottom:18 }}><div><div className="abundance-mini-label">ASSET LIBRARY</div><h2 style={{ fontSize:26, letterSpacing:'-.06em', marginTop:5 }}>Every real output, ready for the next campaign.</h2></div><label className="studio-button" style={{ display:'inline-flex', alignItems:'center', justifyContent:'center' }}>{uploading ? 'Uploading...' : 'Add brand asset'}<input type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={event => handleUpload(event.target.files)} style={{ display:'none' }} /></label></div>{handoffState.message && <div style={{ margin:'0 0 14px', color:handoffState.status === 'done' ? 'var(--signal)' : handoffState.status === 'error' ? '#ffb4a2' : 'rgba(243,240,231,.76)', fontSize:12 }}>{handoffState.message}</div>}{loadingAssets ? <div style={{ minHeight:280, display:'grid', placeItems:'center' }}><span className="spinner" /></div> : assets.length ? <div className="asset-grid">{assets.map(asset => <div key={asset.name} className="asset-card"><AssetVisual asset={asset} compact /><div className="asset-footer"><div style={{ minWidth:0 }}><div style={{ font:'500 9px DM Mono,monospace', color:asset.kind === 'video' ? '#d9ff75':'#ffb5cf', letterSpacing:'.09em' }}>{asset.kind === 'video' ? 'VIDEO RENDER' : 'IMAGE CREATIVE'}</div><div style={{ fontSize:11, fontWeight:700, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:110, marginTop:3 }}>{asset.name}</div></div><div style={{ display:'flex', gap:5 }}><button onClick={() => sendCreativeToReview(asset)} disabled={Boolean(asset.campaign_post_id) || handoffState.status === 'working'} style={{ border:'1px solid rgba(201,242,93,.55)', background:'rgba(201,242,93,.18)', color:'var(--signal)', borderRadius:8, padding:'6px 8px', fontSize:10, fontWeight:800 }}>{asset.campaign_post_id ? 'In review' : 'Review'}</button><button onClick={() => deleteAsset(asset)} style={{ border:'1px solid rgba(255,255,255,.18)', background:'rgba(8,6,28,.72)', color:'#fff', borderRadius:8, padding:'6px 8px', fontSize:10, fontWeight:700 }}>Delete</button></div></div></div>)}</div> : <div className="abundance-glass" style={{ minHeight:260, display:'grid', placeItems:'center', borderRadius:16, color:'rgba(234,229,255,.68)', textAlign:'center', padding:20 }}>Upload your product visual or make the first image ad. FloStudio will keep real assets here for reuse across image and video campaigns.</div>}</section>}
    </div>
  </Layout>
}
