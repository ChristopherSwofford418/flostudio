    ...
    <div className="flo-page" style={{ maxWidth:1280, margin:'0 auto', animation:'fadeIn .3s ease-out' }}>
      ...
      {activeTab === 'generate' && <div className="creative-workspace" style={{ display:'grid', gridTemplateColumns:'minmax(0,1.12fr) minmax(360px,.88fr)', gap:20 }}>
        <section className="abundance-card" style={{ padding:25 }}>
          ...
          <button onClick={generateImages} disabled={generating || (!prompt.trim() && !referenceImage && !activeApp)} className="studio-button" style={{ width:'100%', marginTop:20, padding:14 }}>{generating ? 'Rendering your ad set...' : `Render ${variations} ${selectedRunbook.label.toLowerCase()} ad${variations > 1 ? 's' : ''}`}</button>
        </section>
