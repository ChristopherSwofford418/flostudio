...
export default function SEO() {
  ...
  useEffect(() => {
    ...
    const requested = apps.find(app => app.id === requestedId)
    const current = apps.find(app => app.id === selectedId)
    const fallback = requested || current || apps.find(app => app.id === activeApp?.id) || apps[0]
    if (fallback?.id && fallback.id !== selectedId) setSelectedId(fallback.id)
  }, [apps, requestedId, selectedId, activeApp?.id])
  ...
  const app = useMemo(() => apps.find(item => item.id === selectedId) || apps[0] || null, [apps, selectedId])
  const blueprint = useMemo(() => app ? generateAppSeoBlueprint(app) : null, [app])
  ...
  const { website, appStore, experiments, readiness, sources, sourceCoverage } = blueprint
