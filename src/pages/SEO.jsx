...
import { articleSlug, createSeoArticle, getSeoDestination, listSeoArticles, saveSeoDestination, updateSeoArticleStatus } from '../lib/seoArticles'
import { buildGroundedSeoArticle } from '../lib/seoArticleBuilder'
import { createSearchVisibilityEvidence, formatEvidenceTime, listSearchVisibilityEvidence } from '../lib/searchVisibilityEvidence'
...
function buildVisibilityPromptStarters(app = {}, blueprint = {}) {
  const appName = String(app?.name || 'this app').trim()
  const site = blueprint?.website || {}
  const themes = Array.isArray(site.keywordThemes) ? site.keywordThemes.filter(Boolean).slice(0, 3) : []
  const category = String(app?.category || app?.sourceFacts?.category || 'mobile app').trim()
  const focus = themes[0] || `${category} app`
  return [
    `What should I look for in a ${focus}?`,
    `What are helpful ways to compare ${focus} options?`,
    `How can ${appName} help with its stated purpose?`,
  ]
}
...
export default function SEO() {
  ...
  const [seoDestination, setSeoDestination] = useState(null)
  const [seoArticles, setSeoArticles] = useState([])
  const [articleLoading, setArticleLoading] = useState(false)
  const [articleError, setArticleError] = useState('')
  const [articleNotice, setArticleNotice] = useState('')
  const [destinationForm, setDestinationForm] = useState({ websiteUrl:'', blogBasePath:'/blog', destinationType:'manual_export', publishMode:'review_only', notes:'' })
  const [visibilityEvidence, setVisibilityEvidence] = useState([])
  const [visibilityLoading, setVisibilityLoading] = useState(false)
  const [visibilitySaving, setVisibilitySaving] = useState(false)
  const [visibilityError, setVisibilityError] = useState('')
  const [visibilityNotice, setVisibilityNotice] = useState('')
  const [visibilityForm, setVisibilityForm] = useState({ engine:'chatgpt', promptText:'', resultUrl:'', citedUrls:'', notes:'' })
  ...
  const visibilityPrompts = useMemo(() => buildVisibilityPromptStarters(app, blueprint), [app, blueprint])
  ...
  useEffect(() => {
    let cancelled = false
    if (!app?.id) { setActionTasks([]); setSeoDestination(null); setSeoArticles([]); return undefined }
    setArticleError('')
    Promise.all([getSeoDestination(app.id), listSeoArticles(app.id)]).then(([destination, articles]) => {
      if (cancelled) return
      setSeoDestination(destination)
      setDestinationForm({ websiteUrl:destination?.website_url || '', blogBasePath:destination?.blog_base_path || '/blog', destinationType:destination?.destination_type || 'manual_export', publishMode:destination?.publish_mode || 'review_only', notes:destination?.notes || '' })
      setSeoArticles(articles || [])
    }).catch(error => { if (!cancelled) setArticleError(error?.message || 'SEO Library data could not be loaded.') })
    return () => { cancelled = true }
  }, [app?.id])

  useEffect(() => {
    let cancelled = false
    if (!app?.id) { setVisibilityEvidence([]); return undefined }
    setVisibilityLoading(true)
    setVisibilityError('')
    listSearchVisibilityEvidence(app.id).then(items => {
      if (!cancelled) setVisibilityEvidence(items || [])
    }).catch(error => {
      if (!cancelled) { setVisibilityEvidence([]); setVisibilityError(error?.message || 'Search and AI visibility evidence could not be loaded.') }
  ...
  const saveDestination = async () => {
    if (!app?.id || !workspaceId) return
    setArticleError('')
    setArticleNotice('')
    try {
      const saved = await saveSeoDestination({ workspaceId, productId:app.id, ...destinationForm })
      setSeoDestination(saved)
      setArticleNotice('Website destination saved in the SEO area. External publishing remains disabled until a CMS connection is added.')
    } catch (error) { setArticleError(error?.message || 'Could not save this app website destination.') }
  }

  const generateSeoArticle = async () => {
    if (!app?.id || !workspaceId || articleLoading) return
    setArticleLoading(true)
    setArticleError('')
    setArticleNotice('')
    try {
      const destination = seoDestination || { website_url:destinationForm.websiteUrl, blog_base_path:destinationForm.blogBasePath }
      const article = buildGroundedSeoArticle({ app, blueprint, destination })
      const saved = await createSeoArticle({ workspaceId, productId:app.id, destinationId:seoDestination?.id, article })
      setSeoArticles(current => [saved, ...current])
      setArticleNotice('Grounded SEO article created in the hidden review library. It is not public and has not been pushed externally.')
    } catch (error) { setArticleError(error?.message || 'Could not create the SEO article draft.') }
    finally { setArticleLoading(false) }
  }

  const queueSeoArticle = async article => {
    if (!article?.id || articleLoading) return
    setArticleLoading(true)
    setArticleError('')
    try {
      const updated = await updateSeoArticleStatus(article.id, 'queued')
      setSeoArticles(current => current.map(item => item.id === updated.id ? updated : item))
      setArticleNotice('Article moved to Push SEO review. No external site was changed.')
    } catch (error) { setArticleError(error?.message || 'Could not queue this SEO article.') }
    finally { setArticleLoading(false) }
  }

  const saveVisibilityEvidence = async () => {
    if (!app?.id || !workspaceId || visibilitySaving) return
    setVisibilitySaving(true)
    setVisibilityError('')
    setVisibilityNotice('')
    try {
      const saved = await createSearchVisibilityEvidence({ workspaceId, productId:app.id, ...visibilityForm })
      setVisibilityEvidence(current => [saved, ...current])
      setVisibilityForm(current => ({ ...current, resultUrl:'', citedUrls:'', notes:'' }))
      setVisibilityNotice('Observation saved for this app. It does not change a website, run a crawl, or publish content.')
    } catch (error) { setVisibilityError(error?.message || 'Could not save this visibility observation.') }
  ...
  return <Layout title="SEO & ASO">
    <style>{`
      ...
      .visibility-prompt-field { grid-column:span 2; }
      ...
      @media (max-width:600px) { .seo-page { padding:20px 16px 42px; } .seo-grid-3, .seo-action-grid { grid-template-columns:1fr; } .visibility-prompt-field { grid-column:span 1; } .seo-task-row { align-items:flex-start; flex-direction:column; } }
    `}</style>
    <div className="seo-page">
      ...
      <section className="studio-panel" style={{ marginTop:16, padding:20, borderColor:'rgba(116,213,239,.34)', background:'linear-gradient(135deg,rgba(19,67,88,.27),rgba(16,16,25,.52))' }}><div style={{ display:'flex', justifyContent:'space-between', gap:14, alignItems:'flex-start', flexWrap:'wrap' }}><div><div className="studio-kicker" style={{ color:'#82dff6' }}>SEARCH + AI VISIBILITY / EVIDENCE ONLY</div><h2 style={{ color:'#fff', fontSize:22, marginTop:6 }}>Observe. Attribute. Improve.</h2><p style={{ color:'rgba(240,240,240,.62)', fontSize:11.5, lineHeight:1.55, marginTop:6, maxWidth:720 }}>Track what you personally observe in Google, Bing, ChatGPT, Claude, Gemini, or Perplexity for {app.name}. Save the exact query, any public result or cited URL, and a short note. FloStudio does not run prompts, crawl destinations, create forum posts, or treat a saved observation as a ranking claim.</p></div><span className="studio-chip" style={{ color:'#82dff6', borderColor:'rgba(130,223,246,.34)' }}>{visibilityEvidence.length} OBSERVATION{visibilityEvidence.length === 1 ? '' : 'S'}</span></div><div style={{ display:'grid', gridTemplateColumns:'repeat(3,minmax(0,1fr))', gap:9, marginTop:15 }}>{visibilityPrompts.map((prompt, index) => <button type="button" key={prompt} onClick={() => setVisibilityForm(current => ({ ...current, promptText:prompt }))} className="studio-button studio-button--soft" style={{ textAlign:'left', minHeight:64, padding:'10px 11px', fontSize:10, lineHeight:1.45 }}><span style={{ display:'block', color:'#82dff6', fontSize:8, letterSpacing:'.08em', marginBottom:5 }}>PROMPT STARTER 0{index + 1}</span>{prompt}</button>)}</div><div className="seo-grid-3" style={{ marginTop:15 }}><label className="portfolio-field" style={{ display:'grid', gap:7 }}><span>Where did you check?</span><select value={visibilityForm.engine} onChange={event => setVisibilityForm(current => ({ ...current, engine:event.target.value }))}><option value="google">Google Search</option><option value="bing">Bing Search</option><option value="chatgpt">ChatGPT</option><option value="claude">Claude</option><option value="gemini">Gemini</option><option value="perplexity">Perplexity</option><option value="other">Other</option></select></label><label className="portfolio-field visibility-prompt-field" style={{ display:'grid', gap:7 }}><span>Query or prompt observed</span><input value={visibilityForm.promptText} onChange={event => setVisibilityForm(current => ({ ...current, promptText:event.target.value }))} placeholder="Paste the exact question you checked" /></label></div><div className="seo-grid-2" style={{ marginTop:11 }}><label className="portfolio-field" style={{ display:'grid', gap:7 }}><span>Public result URL (optional)</span><input value={visibilityForm.resultUrl} onChange={event => setVisibilityForm(current => ({ ...current, resultUrl:event.target.value }))} placeholder="https://..." /></label><label className="portfolio-field" style={{ display:'grid', gap:7 }}><span>Cited source URLs (optional)</span><input value={visibilityForm.citedUrls} onChange={event => setVisibilityForm(current => ({ ...current, citedUrls:event.target.value }))} placeholder="One or more URLs, separated by commas" /></label></div><label className="portfolio-field" style={{ display:'grid', gap:7, marginTop:11 }}><span>Observation notes</span><textarea value={visibilityForm.notes} onChange={event => setVisibilityForm(current => ({ ...current, notes:event.target.value }))} placeholder="What did you observe? Keep it factual and dated; do not enter customer or confidential data." rows={2} /></label><div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap', marginTop:12 }}><button type="button" onClick={saveVisibilityEvidence} disabled={visibilitySaving || !visibilityForm.promptText.trim()} className="studio-button" style={{ padding:'8px 11px', fontSize:10 }}>{visibilitySaving ? 'Saving…' : 'Save observed evidence'}</button><span style={{ color:'rgba(240,240,240,.5)', fontSize:10.5 }}>Internal evidence only. No external action is triggered.</span></div>{visibilityError && <p style={{ color:'#ffb6c1', fontSize:11, marginTop:11 }}>{visibilityError}</p>}{visibilityNotice && <p style={{ color:'#9ee8d2', fontSize:11, marginTop:11 }}>{visibilityNotice}</p>}<div style={{ marginTop:18, paddingTop:14, borderTop:'1px solid rgba(240,240,240,.11)' }}><div className="studio-kicker" style={{ color:'rgba(240,240,240,.5)' }}>SELECTED APP EVIDENCE LOG</div>{visibilityLoading && <p style={{ color:'rgba(240,240,240,.5)', fontSize:10.5, marginTop:10 }}>Loading observations…</p>}{!visibilityLoading && !visibilityEvidence.length && <p style={{ color:'rgba(240,240,240,.5)', fontSize:10.5, lineHeight:1.5, marginTop:10 }}>No visibility observations are stored for this app yet. Use a prompt starter, check manually in an approved search or AI tool, and save only what you observed.</p>}{visibilityEvidence.slice(0,6).map(item => <article key={item.id} style={{ padding:'11px 0', borderBottom:'1px solid rgba(240,240,240,.09)' }}><div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center' }}><span className="studio-chip" style={{ color:'#82dff6', borderColor:'rgba(130,223,246,.28)', fontSize:8 }}>{item.engine}</span><span style={{ color:'rgba(240,240,240,.47)', fontSize:9.5 }}>{formatEvidenceTime(item.observed_at)}</span></div><b style={{ display:'block', color:'#fff', fontSize:11.5, lineHeight:1.5, marginTop:6 }}>{item.prompt_text}</b>{item.notes && <p style={{ color:'rgba(240,240,240,.57)', fontSize:10, lineHeight:1.45, marginTop:4 }}>{item.notes}</p>}<div style={{ display:'flex', gap:8, flexWrap:'wrap', marginTop:7 }}>{item.result_url && <a href={item.result_url} target="_blank" rel="noreferrer" className="studio-chip" style={{ fontSize:8.5, color:'#d8f7ff' }}>Open observed result ↗</a>}{(item.cited_urls || []).slice(0,4).map((url, index) => <a href={url} target="_blank" rel="noreferrer" key={url} className="studio-chip" style={{ fontSize:8.5, color:'rgba(240,240,240,.75)' }}>Citation {index + 1} ↗</a>)}</div></article>)}</div><div style={{ marginTop:15, padding:12, borderRadius:10, border:'1px solid rgba(130,223,246,.18)', background:'rgba(130,223,246,.055)' }}><b style={{ color:'#d8f7ff', fontSize:10.5 }}>Responsible visibility boundary</b><p style={{ color:'rgba(240,240,240,.58)', fontSize:10, lineHeight:1.5, marginTop:5 }}>This workspace supports factual research, content-gap planning, and human editorial review. It does not promise ranking or citation outcomes, publish content, change robots.txt, submit a sitemap, post to Reddit, or use accounts to manipulate search or AI answers.</p></div></section>

      <section className="studio-panel" style={{ marginTop:16, padding:20, borderColor:'rgba(126,225,197,.25)', background:'linear-gradient(135deg,rgba(26,61,65,.2),rgba(16,16,25,.5))' }}><div style={{ display:'flex', justifyContent:'space-between', gap:14, alignItems:'flex-start', flexWrap:'wrap' }}><div><div className="studio-kicker" style={{ color:'var(--signal)' }}>SEO DESTINATION / {app.name.toUpperCase()}</div><h2 style={{ color:'#fff', fontSize:22, marginTop:6 }}>Create articles here. Push them when ready.</h2><p style={{ color:'rgba(240,240,240,.62)', fontSize:11.5, lineHeight:1.55, marginTop:6, maxWidth:720 }}>This private SEO Library is inside Flo Studio and is not publicly indexed. Enter the selected app’s website so every article gets the right destination context. A URL alone does not publish externally; CMS or webhook connection can be added later.</p></div><span className="studio-chip" style={{ color:seoDestination?.website_url ? 'var(--signal)' : '#cbc8ff', borderColor:seoDestination?.website_url ? 'rgba(126,225,197,.3)' : 'rgba(203,200,255,.3)' }}>{seoDestination?.website_url ? 'DESTINATION SAVED' : 'DESTINATION NEEDED'}</span></div><div className="seo-grid-3" style={{ marginTop:16 }}><label className="portfolio-field" style={{ display:'grid', gap:7 }}><span>Website URL</span><input value={destinationForm.websiteUrl} onChange={event => setDestinationForm(current => ({ ...current, websiteUrl:event.target.value }))} placeholder="https://your-app-site.com" /></label><label className="portfolio-field" style={{ display:'grid', gap:7 }}><span>Blog base path</span><input value={destinationForm.blogBasePath} onChange={event => setDestinationForm(current => ({ ...current, blogBasePath:event.target.value }))} placeholder="/blog" /></label><label className="portfolio-field" style={{ display:'grid', gap:7 }}><span>Destination type</span><select value={destinationForm.destinationType} onChange={event => setDestinationForm(current => ({ ...current, destinationType:event.target.value }))}><option value="manual_export">Manual export / CMS handoff</option><option value="wordpress">WordPress</option><option value="webflow">Webflow</option><option value="shopify">Shopify</option><option value="webhook">CMS webhook</option><option value="custom_cms">Custom CMS</option></select></label></div><div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap', marginTop:12 }}><button type="button" onClick={saveDestination} className="studio-button studio-button--soft" style={{ padding:'8px 11px', fontSize:10 }}>Save app destination</button><span style={{ color:'rgba(240,240,240,.5)', fontSize:10.5 }}>Publish mode: review-only until an approved destination connection exists.</span></div><div style={{ marginTop:22, paddingTop:17, borderTop:'1px solid rgba(240,240,240,.11)' }}><div style={{ display:'flex', justifyContent:'space-between', gap:12, alignItems:'center', flexWrap:'wrap' }}><div><div className="studio-kicker" style={{ color:'var(--signal)' }}>HIDDEN SEO ARTICLE LIBRARY / {app.name.toUpperCase()}</div><p style={{ color:'rgba(240,240,240,.58)', fontSize:10.5, lineHeight:1.5, marginTop:5 }}>Generate grounded articles from this app’s saved plan, then move an article to Push SEO review. Nothing becomes public from this button.</p></div><button type="button" onClick={generateSeoArticle} disabled={articleLoading} className="studio-button" style={{ padding:'9px 12px', fontSize:10 }}>{articleLoading ? 'Working…' : 'Generate grounded SEO blog →'}</button></div>{articleError && <p style={{ color:'#ffb6c1', fontSize:11, marginTop:11 }}>{articleError}</p>}{articleNotice && <p style={{ color:'#9ee8d2', fontSize:11, marginTop:11 }}>{articleNotice}</p>}{!seoArticles.length && <div style={{ marginTop:13, padding:14, border:'1px dashed rgba(240,240,240,.2)', borderRadius:11, color:'rgba(240,240,240,.55)', fontSize:11, lineHeight:1.55 }}>No private SEO articles have been created for this app yet. Generate one from the grounded plan above.</div>}{seoArticles.map(article => <article key={article.id} style={{ marginTop:13, padding:14, border:'1px solid rgba(240,240,240,.12)', borderRadius:12, background:'rgba(15,15,25,.4)' }}><div style={{ display:'flex', justifyContent:'space-between', gap:12, alignItems:'flex-start', flexWrap:'wrap' }}><div style={{ minWidth:0 }}><div className="studio-kicker" style={{ color:'rgba(240,240,240,.5)' }}>{article.focus_keyword || 'GROUNDED ARTICLE'} · /{article.slug}</div><h3 style={{ color:'#fff', fontSize:15, lineHeight:1.35, marginTop:6 }}>{article.title}</h3><p style={{ color:'rgba(240,240,240,.58)', fontSize:10.5, lineHeight:1.5, marginTop:5 }}>{article.excerpt}</p></div><span className="studio-chip" style={{ color:article.status === 'queued' ? '#ffc98a' : '#cbc8ff', borderColor:'rgba(203,200,255,.25)' }}>{article.status.replaceAll('_',' ')}</span></div><div style={{ display:'flex', gap:8, flexWrap:'wrap', marginTop:12 }}><button type="button" onClick={() => queueSeoArticle(article)} disabled={articleLoading || ['queued','pushed'].includes(article.status)} className="studio-button studio-button--soft" style={{ padding:'8px 10px', fontSize:10 }}>{article.status === 'queued' ? 'Queued for Push SEO' : article.status === 'pushed' ? 'Pushed' : 'Push SEO → review'}</button><button type="button" onClick={() => navigator.clipboard?.writeText(article.content_markdown || '')} className="studio-button studio-button--soft" style={{ padding:'8px 10px', fontSize:10 }}>Copy article markdown</button></div></article>)}</div></section>
