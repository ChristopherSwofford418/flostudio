  ...
  return <Layout title="Agent Studio">
    ...
    <div className="flo-page" style={{ maxWidth: 1360, margin: '0 auto', animation: 'agentEnter .42s var(--ease-out)' }}>
      ...
      <div className="agent-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.55fr) minmax(320px,.72fr)', gap: 20, alignItems: 'start' }}>
        <section ref={briefRef} className="studio-panel" style={{ overflow: 'hidden', background:'linear-gradient(145deg,#1d173d 0%,#100d26 72%)', borderColor:'rgba(201,184,255,.18)', boxShadow:'0 24px 70px rgba(5,3,20,.34)' }}>
          <div style={{ padding: '25px 28px 0', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
            <div><span className="studio-kicker" style={{ color:'#d9ff75' }}>Campaign brief</span><h2 style={{ fontSize: 25, letterSpacing: '-.055em', marginTop: 7, color:'#fff' }}>Give your creative a point of view.</h2></div>
            <span className="studio-chip" style={{ color:'#ece6ff', borderColor:'rgba(255,255,255,.16)', background:'rgba(255,255,255,.06)' }}>{selectedPlatforms.length} channels live</span>
          </div>
          <div className="brief-grid" style={{ display: 'grid', gridTemplateColumns: '150px 1fr', gap: 0, marginTop: 25, borderTop: '1px solid var(--line)' }}>
            <div style={{ padding: 22, borderRight: '1px solid rgba(255,255,255,.1)', background: 'rgba(1,1,13,.25)' }}>{stages.map(([number, title, detail], index) => <div key={number} style={{ padding: '12px 0', opacity: activeStep === index || activeStep > index || (activeStep === -1 && index === 0) ? 1 : .42, borderBottom: index < stages.length - 1 ? '1px solid rgba(255,255,255,.1)' : 'none' }}><div style={{ fontFamily: 'DM Mono, monospace', fontSize: 10, color: index === activeStep ? '#ff8a69' : 'rgba(234,229,255,.58)' }}>{number}</div><div style={{ fontSize: 11.5, fontWeight: 800, marginTop: 3, color:'#fff' }}>{title}</div><div style={{ fontSize: 10, color: 'rgba(234,229,255,.54)', lineHeight: 1.4, marginTop: 2 }}>{detail}</div></div>)}</div>
            <div style={{ padding: 25, display: 'grid', gap: 20 }}>
              <label style={{ display: 'grid', gap: 8 }}><span style={{ fontSize: 12, fontWeight: 800, color:'#fff' }}>What are you building momentum for?</span><textarea className="studio-input" value={brand} onChange={e => setBrand(e.target.value)} rows={4} placeholder="Describe the product, launch, offer, or market moment in your own words." style={{ resize: 'vertical', lineHeight: 1.6, background:'rgba(5,4,20,.48)', borderColor:'rgba(255,255,255,.14)', color:'#fff' }} /></label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}><label style={{ display: 'grid', gap: 8 }}><span style={{ fontSize: 12, fontWeight: 800, color:'#fff' }}>Who needs to care?</span><input className="studio-input" value={audience} onChange={e => setAudience(e.target.value)} placeholder="Audience and context" style={{ background:'rgba(5,4,20,.48)', borderColor:'rgba(255,255,255,.14)', color:'#fff' }} /></label><label style={{ display: 'grid', gap: 8 }}><span style={{ fontSize: 12, fontWeight: 800, color:'#fff' }}>What should change?</span><input className="studio-input" value={goal} onChange={e => setGoal(e.target.value)} placeholder="Awareness, leads, revenue…" style={{ background:'rgba(5,4,20,.48)', borderColor:'rgba(255,255,255,.14)', color:'#fff' }} /></label></div>
              <div><div style={{ fontSize: 12, fontWeight: 800, marginBottom: 9, color:'#fff' }}>Select the places this needs to land</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>{platforms.map(platform => <button key={platform} onClick={() => togglePlatform(platform)} className={`studio-chip ${selectedPlatforms.includes(platform) ? 'active' : ''}`}>{platformLabel[platform]}</button>)}</div></div>
              <div style={{ padding:14, borderRadius:13, background:'rgba(255,255,255,.055)', border:'1px solid rgba(203,189,255,.17)' }}><div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}><div><div style={{ fontSize:12, fontWeight:800, color:'#fff' }}>Campaign visual</div><div style={{ fontSize:10.5, color:'rgba(234,229,255,.58)', marginTop:3 }}>{selectedMediaAsset ? 'This real asset will attach to the lead post in your new campaign.' : 'Choose a Creative Lab asset to carry into the campaign.'}</div></div><button onClick={() => navigate('/images')} className="studio-chip" style={{ color:'#ece6ff', borderColor:'rgba(255,255,255,.15)', background:'rgba(255,255,255,.08)' }}>Open Creative Lab</button></div>{mediaAssets.length ? <div style={{ display:'flex', gap:7, overflowX:'auto', marginTop:10 }}>{mediaAssets.slice(0,6).map(asset => <button key={asset.id} onClick={() => setSelectedMediaAsset(selectedMediaAsset?.id === asset.id ? null : asset)} style={{ width:54, height:54, padding:0, borderRadius:9, overflow:'hidden', flexShrink:0, cursor:'pointer', border:selectedMediaAsset?.id === asset.id ? '3px solid #d7f267' : '1px solid rgba(255,255,255,.2)', background:'#17122f' }}>{asset.kind === 'video' ? <video src={asset.asset_url} muted playsInline preload="metadata" style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <img src={asset.asset_url} alt="Campaign visual choice" style={{ width:'100%', height:'100%', objectFit:'cover' }} />}</button>)}</div> : <div style={{ fontSize:10.5, color:'rgba(234,229,255,.5)', marginTop:9 }}>No saved assets yet. Make the first image or video in Creative Lab.</div>}</div>
              <div style={{ padding: 16, background: 'rgba(124,97,255,.13)', borderRadius: 14, border: '1px solid rgba(193,181,255,.18)' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}><span style={{ fontSize: 12, fontWeight: 800, color:'#fff' }}>Creative frequency</span><span style={{ fontFamily: 'DM Mono, monospace', color: '#d9ff75', fontSize: 12 }}>{volume} / week</span></div><input type="range" min={3} max={14} value={volume} onChange={e => setVolume(Number(e.target.value))} style={{ width: '100%', accentColor: '#d7f267' }} /></div>
