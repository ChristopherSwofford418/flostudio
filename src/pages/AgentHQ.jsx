...
const fieldStyle = { width:'100%', boxSizing:'border-box', background:'rgba(48,8,3,.48)', border:'1px solid rgba(255,247,230,.18)', borderRadius:3, color:'#fff', padding:'12px 13px', outline:'none', font:'inherit', fontSize:12.5, lineHeight:1.55 }
  ...
  return <Layout title="Campaign Engine">
    ...
    <div className="flo-page engine-ledger" style={{ maxWidth:1360, margin:'0 auto', animation:'engineEnter .42s var(--ease-out)' }}>
      <section className="studio-dark engine-header" style={{ position:'relative', overflow:'hidden', minHeight:296, padding:'37px 42px', marginBottom:18 }}>
        <div style={{ position:'absolute', width:420, height:420, right:-150, top:-230, borderRadius:'50%', background:'radial-gradient(circle,#ffc13b 0%,#ff6a35 32%,rgba(255,106,53,0) 70%)', opacity:.70 }} />
        <div style={{ position:'absolute', width:240, height:240, right:120, bottom:-130, borderRadius:3, background:'linear-gradient(140deg,#ffc13b,#ff5d32)', transform:'rotate(24deg)', opacity:.76 }} />
        <div style={{ position:'relative', maxWidth:800 }}>
          ...
          <p style={{ maxWidth:630, marginTop:16, color:'rgba(243,240,231,.74)', fontSize:13.5, lineHeight:1.7 }}>Flo learns the product and its rules first, then creates campaign angles, review-ready platform posts, and real creative variants that stay connected to the campaign.</p>
          <div className="engine-actions" style={{ position:'absolute', right:-335, bottom:3, display:'flex', gap:8 }}><span className="studio-chip" style={{ background:'rgba(243,240,231,.1)', borderColor:'rgba(243,240,231,.2)', color:'#fff' }}>{tokens} creative tokens</span><button onClick={startOver} className="studio-chip" style={{ background:'var(--signal)', borderColor:'var(--signal)', color:'var(--ink-deep)' }}>New campaign</button></div>
        </div>
      </section>
      ...
      <div className="engine-grid" style={{ display:'grid', gridTemplateColumns:'178px minmax(0,1fr) 290px', gap:18, alignItems:'start' }}>
        <aside className="engine-rail studio-panel" style={{ padding:'16px', background:'linear-gradient(160deg,#661a0a,#350804)', borderColor:'rgba(255,247,230,.15)' }}>
          <div className="studio-kicker" style={{ color:'#ffc13b', marginBottom:14 }}>Campaign run</div>
        ...
        <main className="studio-panel" style={{ overflow:'hidden', background:'linear-gradient(145deg,#731e0d,#390804)', borderColor:'rgba(255,193,59,.28)' }}>
          {stage === 'intake' && <section style={{ padding:'26px 28px' }}>
            <div className="studio-kicker" style={{ color:'#ffc13b' }}>01 / Product intake</div><h2 style={{ color:'#fff', fontSize:27, letterSpacing:'-.06em', marginTop:7 }}>What are we taking to market?</h2><p style={{ color:'rgba(255,247,230,.68)', fontSize:12, lineHeight:1.65, marginTop:8, maxWidth:650 }}>Start with a live product page, app listing, or your own facts. Flo uses only the details you confirm in the next step.</p>
            <div style={{ display:'grid', gap:16, marginTop:23 }}>
              ...
              {urlState && <div style={{ padding:'10px 12px', borderRadius:10, background:'rgba(99,183,255,.12)', border:'1px solid rgba(99,183,255,.30)', color:'#9fd2ff', fontSize:11.5 }}>{urlState}</div>}
        ...
        <aside style={{ display:'grid', gap:18 }}>
          ...
          <section className="studio-panel engine-memory" style={{ padding:18, background:'linear-gradient(145deg,#122b4d,#193b69)', borderColor:'rgba(99,183,255,.38)' }}><div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:8 }}><div className="studio-kicker" style={{ color:'#9fd2ff' }}>Creative Memory</div><span style={{ width:8, height:8, borderRadius:'50%', background:'#63b7ff', boxShadow:'0 0 14px #63b7ff' }} /></div>{memoryBrief ? <><div style={{ color:'#fff', fontSize:15, fontWeight:850, letterSpacing:'-.045em', marginTop:12, lineHeight:1.25 }}>{memoryBrief.headline}</div><div style={{ color:'#9fd2ff', fontSize:10.5, fontWeight:800, marginTop:10, lineHeight:1.5 }}>{memoryBrief.state}</div><p style={{ color:'rgba(238,247,255,.76)', fontSize:10.5, lineHeight:1.55, marginTop:7 }}>{memoryBrief.nextAction}</p><div style={{ borderTop:'1px solid rgba(255,255,255,.11)', marginTop:12, paddingTop:10, color:'rgba(238,247,255,.58)', fontSize:9.5, lineHeight:1.5 }}>{memoryBrief.rationale}</div>{memoryBrief.evidence?.length ? <div style={{ display:'flex', gap:5, flexWrap:'wrap', marginTop:10 }}>{memoryBrief.evidence.map(item => <span key={item} style={{ border:'1px solid rgba(99,183,255,.32)', color:'#cde8ff', background:'rgba(99,183,255,.11)', padding:'4px 6px', borderRadius:6, fontSize:8.5 }}>{item}</span>)}</div> : null}</> : <p style={{ color:'rgba(238,247,255,.66)', fontSize:10.5, lineHeight:1.55, marginTop:10 }}>Flo will show evidence-linked creative guidance here after the first brand, campaign, or review decision.</p>}</section>
          <section className="studio-panel engine-recent" style={{ padding:18, background:'rgba(104,25,9,.86)', borderColor:'rgba(255,247,230,.13)' }}><div className="studio-kicker" style={{ color:'#ffc13b' }}>Recent campaign memory</div>{workspace.campaigns.length ? <div style={{ display:'grid', gap:9, marginTop:12 }}>{workspace.campaigns.slice(0,4).map(item => <button key={item.id} onClick={() => openExistingCampaign(item)} style={{ textAlign:'left', background:'transparent', border:'none', borderTop:'1px solid rgba(255,255,255,.1)', padding:'10px 0', cursor:'pointer', fontFamily:'inherit' }}><div style={{ fontSize:11.5, color:'#fff', fontWeight:800, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{item.name}</div><div style={{ fontSize:9.5, color:'rgba(255,247,230,.61)', marginTop:3 }}>{item.status.replaceAll('_',' ')} · {new Date(item.created_at).toLocaleDateString()}</div></button>)}</div> : <p style={{ color:'rgba(255,247,230,.61)', fontSize:11, lineHeight:1.55, marginTop:10 }}>The campaigns you build here become reusable institutional memory for each brand and product.</p>}</section>
        </aside>
      </div>
    </div>
  </Layout>
