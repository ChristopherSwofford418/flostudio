...
function providerSetup() {
  ...
  const secureStorage = Boolean(process.env.SUPABASE_SERVICE_ROLE_KEY && (process.env.UNIFIED_SOCIAL_VAULT_KEY || process.env.SOCIAL_CREDENTIALS_ENCRYPTION_KEY || process.env.OPENAI_PROVIDER_VAULT_KEY))
  return {
    configured: Boolean(apiKey && domain && privateKey && secureStorage),
    apiKey,
    domain,
    privateKey,
    secureStorage,
    requirement: 'Add AYRSHARE_API_KEY, AYRSHARE_DOMAIN, AYRSHARE_PRIVATE_KEY, SUPABASE_SERVICE_ROLE_KEY, and a server-side vault key in FloStudio production. Then users can securely connect their own social accounts.',
  }
}
...
async function authenticatedUser(req) {
  ...
  if (!token) throw apiError('AUTH_REQUIRED', 'Sign in to FloStudio before managing social connections.', 401)
  const response = await fetch(`${SUPABASE_URL}/auth/v1/user`, { headers:{ Authorization:`Bearer ${token}`, apikey:required(process.env.SUPABASE_SERVICE_ROLE_KEY, 'SUPABASE_SERVICE_ROLE_KEY') } })
...
async function db(path, { method='GET', body, prefer='return=representation' } = {}) {
  ...
  if (!response.ok) throw apiError('UNIFIED_SOCIAL_DATABASE_ERROR', payload?.message || 'FloStudio could not update the protected social configuration.', 500, { providerStatus:response.status })
  return payload
}

async function productForUser(userId, productId) {
  ...
  const rows = await db(`products?${params.toString()}`)
...
async function profileFor(userId) {
  ...
  const rows = await db(`unified_social_profiles?${params.toString()}`)
  return rows?.[0] || null
}
...
async function ensureProfile(user, workspaceId, setup) {
...
async function syncProfile(user, setup) {
  ...
  const rows = await db(`unified_social_profiles?id=eq.${encodeURIComponent(profile.id)}`, { method:'PATCH', body:{ status:accounts.length ? 'connected':'connection_pending', connected_platforms:Array.from(new Set(accounts.map(account => account.platform))), account_snapshot:snapshot, last_synced_at:new Date().toISOString(), last_error_code:null, last_error_message:null, updated_at:new Date().toISOString() } })
  return { profile:rows?.[0] || profile, accounts, configured:true }
}

async function appConfig(user, productId) {
  const product = await productForUser(user.id, productId)
  const [agentRows, channelRows] = await Promise.all([
    db(`app_brand_agents?select=*&user_id=eq.${user.id}&product_id=eq.${encodeURIComponent(product.id)}&limit=1`),
    db(`app_channel_profiles?select=*&user_id=eq.${user.id}&product_id=eq.${encodeURIComponent(product.id)}&order=platform.asc`),
  ])
...
async function saveBrandAgent(user, body) {
  const product = await productForUser(user.id, body.productId)
  ...
  const rows = await db('app_brand_agents?on_conflict=user_id,product_id', { method:'POST', prefer:'resolution=merge-duplicates,return=representation', body:payload })
  return { agent:rows?.[0], context }
}

async function saveChannelProfile(user, body) {
  const product = await productForUser(user.id, body.productId)
  ...
  const unified = await profileFor(user.id)
  ...
  const rows = await db('app_channel_profiles?on_conflict=user_id,product_id,platform', { method:'POST', prefer:'resolution=merge-duplicates,return=representation', body:payload })
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (body.action === 'begin_connect') {
      const profile = await ensureProfile(user, body.workspaceId, setup)
      if (!setup.configured) throw apiError('UNIFIED_SOCIAL_SETUP_REQUIRED', setup.requirement, 503)
      ...
      await db(`unified_social_profiles?id=eq.${encodeURIComponent(profile.id)}`, { method:'PATCH', body:{ status:'connection_pending', updated_at:new Date().toISOString() } })
    ...
    if (body.action === 'sync') return res.status(200).json(await syncProfile(user, setup))
    if (body.action === 'app_config') return res.status(200).json(await appConfig(user, body.productId))
    if (body.action === 'save_brand_agent') return res.status(200).json(await saveBrandAgent(user, body))
    if (body.action === 'save_channel') return res.status(200).json(await saveChannelProfile(user, body))
